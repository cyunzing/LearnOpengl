...
int main(int argc, char *argv[])
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUniform3f(glGetUniformLocation(shader.programId, "light.position"), camera.position.x, camera.position.y, camera.position.z);
        glUniform3f(glGetUniformLocation(shader.programId, "light.direction"), camera.forward.x, camera.forward.y, camera.forward.z);
        glUniform1f(glGetUniformLocation(shader.programId, "light.cutoff"), cos(glm::radians(12.5f)));
        glUniform1f(glGetUniformLocation(shader.programId, "light.outerCutoff"), cos(glm::radians(17.5f)));
