...
int main(int argc, char *argv[])
{
	...
	GLFWwindow *window = glfwCreateWindow(WINDOW_SIZE, WINDOW_SIZE, "Stencil planar reflection", NULL, NULL);
	if (!window) {
	...
	Shader planeShader("singlecolor.vert", "singlecolor.frag");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		planeShader.use();
		glUniformMatrix4fv(glGetUniformLocation(planeShader.programId, "projection"), 1, GL_FALSE, glm::value_ptr(projection));
		glUniformMatrix4fv(glGetUniformLocation(planeShader.programId, "view"), 1, GL_FALSE, glm::value_ptr(view));
		model = glm::mat4();
		glUniformMatrix4fv(glGetUniformLocation(planeShader.programId, "model"), 1, GL_FALSE, glm::value_ptr(model));
		glBindVertexArray(planeVaoId);
		...
		secondReflectModel = glm::translate(secondReflectModel, glm::vec3(0.0f, 0.5f, 0.0f));
		glUniformMatrix4fv(glGetUniformLocation(shader.programId, "model"), 1, GL_FALSE, glm::value_ptr(secondReflectModel));
